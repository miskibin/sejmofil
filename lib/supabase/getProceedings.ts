...
export const getPopularProceedingPoints = cache(
  async (): Promise<LatestPointsResult[]> => {
    ...
    if (pointsRes.error) {
      console.error(
        'Error fetching popular proceeding points:',
        pointsRes.error
      )
      return []
    }
...
export const getForYouProceedingPoints = cache(
  async (): Promise<LatestPointsResult[]> => {
    const supabase = await createClient()
    ...
    if (pointsRes.error) {
      console.error(
        'Error fetching for you proceeding points:',
        pointsRes.error
      )
      return []
    }
    ...
    const votesByPointId = voteCounts.reduce(
      ...
      {} as Record<
        number,
        { sumOfVotes: number; votes: { upvotes: number; downvotes: number } }
      >
    )
